...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    if not datastore['PAYLOAD_DLL'].downcase.match(/\.dll$/i)
      fail_with(Failure::BadConfig, 'The payload file path is not valid!')
    end

    @proto = (datastore['SSL'] ? 'https' : 'http')
    if datastore['PAYLOAD_PREFIX'].nil? or datastore['PAYLOAD_PREFIX'].empty?
    ...
    if File.exist?(datastore['PAYLOAD_DLL']) and not File.zero?(datastore['PAYLOAD_DLL'])
